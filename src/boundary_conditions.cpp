...
template<int dim>
void Ddhdg::BoundaryConditionHandler<dim>::add_boundary_condition(dealii::types::boundary_id id,
        const Ddhdg::DirichletBoundaryCondition<dim>& db)
{
    if (dbc_map[id].find(db.get_component())!=dbc_map[id].end()) {
        std::string error = "On boundary id "+std::to_string(id)+", two "+
...
template<int dim>
void Ddhdg::BoundaryConditionHandler<dim>::add_boundary_condition(dealii::types::boundary_id id,
        BoundaryConditionType bc_type, Component c, std::shared_ptr<const dealii::Function<dim>> f)
{
...
template<int dim>
bool Ddhdg::BoundaryConditionHandler<dim>::has_dirichlet_boundary_conditions(dealii::types::boundary_id id) const
{
    return dbc_map.find(id)!=dbc_map.end();
}

template<int dim>
bool Ddhdg::BoundaryConditionHandler<dim>::has_neumann_boundary_conditions(dealii::types::boundary_id id) const
{
...
Ddhdg::BoundaryConditionHandler<dim>::get_dirichlet_conditions_for_id(dealii::types::boundary_id id) const
{
...
Ddhdg::BoundaryConditionHandler<dim>::get_neumann_conditions_for_id(dealii::types::boundary_id id) const
{
    return nbc_map.at(id);
}

template
class Ddhdg::BoundaryConditionHandler<1>;

template
class Ddhdg::BoundaryConditionHandler<2>;

template
class Ddhdg::BoundaryConditionHandler<3>;
