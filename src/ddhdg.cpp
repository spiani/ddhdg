...
namespace Ddhdg {
    ...
    Solver<dim>::Solver(
            ...
            : triangulation(problem.triangulation)
              ...
              , f(problem.f)
              , fe_local(
                    FE_DGQ<dim>(v_degree), dim, FE_DGQ<dim>(v_degree), 1,
                    FE_DGQ<dim>(n_degree), dim, FE_DGQ<dim>(n_degree), 1)
    ...
    void Solver<dim>::setup_system()
    {
        ...
        /*if (boundary_handler->has_dirichlet_boundary_conditions()) {
            ...
            for (auto const& boundary_map : dirichlet_boundary_maps) {
                auto component = boundary_map.first;
                auto boundary_function_map = boundary_map.second;
            ...
            PerTaskData& task_data)
    {
        ...
        for (unsigned int q = 0; q<n_q_points; ++q) {
            ...
            for (unsigned int k = 0; k<loc_dofs_per_cell; ++k) {
                ...
                scratch.V[k] = scratch.fe_values_local[electric_potential].value(k, q);
                scratch.V_grad[k] =
                        scratch.fe_values_local[electric_potential].gradient(k, q);

                scratch.W[k] = scratch.fe_values_local[electron_displacement].value(k, q);
                scratch.W_div[k] =
                        scratch.fe_values_local[electron_displacement].divergence(k, q);
                scratch.n[k] = scratch.fe_values_local[electron_density].value(k, q);
                scratch.n_grad[k] =
            ...
            for (unsigned int i = 0; i<loc_dofs_per_cell; ++i) {
                ...
                scratch.l_rhs(i) += scratch.V[i]*rhs_value*JxW+
                        +scratch.n[i]*rhs_value*JxW;
            }
        }

        // Integrals on the cell border
        for (unsigned int face = 0; face<GeometryInfo<dim>::faces_per_cell; ++face) {
            const dealii::types::boundary_id face_boundary_id = cell->face(face)->boundary_id();
            scratch.fe_face_values_local.reinit(loc_cell, face);
            scratch.fe_face_values.reinit(cell, face);
            if (task_data.trace_reconstruct) {
                scratch.fe_face_values[electric_field_trace].get_function_values(solution,
                        scratch.tr_V_solution_values);
                scratch.fe_face_values[electron_density_trace].get_function_values(solution,
                        scratch.tr_n_solution_values);
            }
            ...
            if (cell->face(face)->at_boundary()) {
                if (boundary_handler->has_dirichlet_boundary_conditions(face_boundary_id)) {
                    dirichlet_boundary_map<dim> boundary_map = boundary_handler->get_dirichlet_conditions_for_id(
                            face_boundary_id);
                ...
                if (boundary_handler->has_neumann_boundary_conditions(face_boundary_id)) {
                    neumann_boundary_map<dim> boundary_map = boundary_handler->get_neumann_conditions_for_id(
                            face_boundary_id);
                    if (boundary_map.find(V)!=boundary_map.end()) {
            ...
            for (unsigned int q = 0; q<n_face_q_points; ++q) {
                ...
                // If there are Dirichlet boundary conditions, evaluate them on the current quadrature point
                if (V_has_dirichlet_conditions || n_has_dirichlet_conditions) {
                    const auto boundary_map = boundary_handler->get_dirichlet_conditions_for_id(face_boundary_id);
                    if (V_has_dirichlet_conditions)
                        V_dbc_value = boundary_map.find(V)->second.evaluate(quadrature_point);
                    if (n_has_dirichlet_conditions)
                        n_dbc_value = boundary_map.find(n)->second.evaluate(quadrature_point);
                }
                ...
                for (unsigned int k = 0; k<scratch.fe_local_support_on_face[face].size(); ++k) {
                    ...
                    scratch.E[k] = scratch.fe_face_values_local[electric_field].value(kk, q);
                    scratch.V[k] = scratch.fe_face_values_local[electric_potential].value(kk, q);
                    scratch.W[k] = scratch.fe_face_values_local[electron_displacement].value(kk, q);
                    scratch.n[k] = scratch.fe_face_values_local[electron_density].value(kk, q);
                }
                if (!task_data.trace_reconstruct) {
                    ...
                    for (unsigned int i = 0; i<scratch.fe_local_support_on_face[face].size(); ++i) {
                        for (unsigned int j = 0; j<scratch.fe_support_on_face[face].size(); ++j) {
                            ...
                            // Here j is the index of the test function (because we are
                ...
                for (unsigned int i = 0;
                    ...
                    for (unsigned int j = 0;
                        ...
                        scratch.ll_matrix(ii, jj) +=
                                (scratch.E[j]*normal+
                                        tau_stab*scratch.V[j])*
                                        scratch.V[i]*JxW;
                        scratch.ll_matrix(ii, jj) +=
                                (scratch.W[j]*normal+
                                        tau_stab*scratch.n[j])*
                                        scratch.n[i]*JxW;
                    }
                ...
                if (task_data.trace_reconstruct)
                    for (unsigned int i = 0;
                        ...
                        scratch.l_rhs(ii) +=
                                (scratch.E[i]*normal+
                                        scratch.V[i]*tau_stab)*
                                        scratch.tr_V_solution_values[q]*JxW;
                        scratch.l_rhs(ii) +=
                                (scratch.W[i]*normal+
                                        scratch.n[i]*tau_stab)*
    ...
    void Solver<dim>::output_results(const std::string& solution_filename)
    {
        ...
        std::vector<DataComponentInterpretation::DataComponentInterpretation>
                ...
                2*(dim+1), DataComponentInterpretation::component_is_part_of_vector);
