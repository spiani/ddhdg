...
namespace Ddhdg
{
    ...
    Solver<dim>::Solver (
        ...
        : triangulation (problem.triangulation), boundary_handler (problem.boundary_handler), f (problem.f), fe_local (FE_DGQ<dim> (degree), dim, FE_DGQ<dim> (degree), 1), dof_handler_local (*triangulation), fe (degree), dof_handler (*triangulation)
    {}
          ...
          for (auto const &boundary_map : dirichlet_boundary_maps)
            {
              ...
              VectorTools::project_boundary_values (dof_handler,
                                                    ...
                                                    constraints,
                                                    get_component_mapping(component));
            }
        }
      constraints.close ();
      {
